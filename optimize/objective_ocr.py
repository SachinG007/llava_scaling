...
def grid_search():
    ...
    b = np.linspace(0.0, 0.5, 100)
    c = np.linspace(0.02, 0.1, 100)
    d = np.linspace(0.0, 0.2, 20)
...
if do_search == "search":
    ...
    with open("best_params_ocr.csv") as f:
        lines = f.readlines()
...
if do_search == "search":
    with open("best_params_ocr.csv", "a") as f:
...
plot_token_values = np.array(np.linspace(4, 576, 50))
...
best_points = [(0.3, 576), (0.5, 576), (1.8, 576), (4, 576), (7, 576), (9, 576), (12, 576), (15, 576), (18, 576), (20, 576)]
...
custom_lines = [
    Line2D([0], [0], color='none', linestyle='None', label=r'$\alpha = 0.029$'),
    Line2D([0], [0], color='none', linestyle='None', label=r'$\beta = 0.048$'),
...
#make legend compact
plt.title(f"Scaling trends for OCR Tasks")
#tight layout
plt.tight_layout()
#save the plot
plt.savefig(f"ocr_scaling_plot_text{TEXT_TOKENS}.png")
plt.savefig(f"ocr_scaling_plot_text{TEXT_TOKENS}.pdf")
