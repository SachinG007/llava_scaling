...
for n_idx, n in enumerate(all_n_values):
    ...
    for plot_idx, flop in enumerate(flops):
        if avg_downstream_error[plot_idx]<0.99:
            if n==14:
                color = 'black'
    ...
    if n==14:
        plt.plot(x, y, color=color, alpha=0.9, linestyle='dotted')    
    else:
...
cbar.set_ticks([0.5, 1.8, 4, 7, 14])
cbar.set_ticklabels(["0.5B", "1.8B", "4B", "7B", "14B"])
...
custom_lines = [
    ...
    Line2D([0], [0], color = "gray", linestyle='-', label='Fitted Curves'),
    Line2D([0], [0], marker='s', markersize=8, color='black', linestyle='None', label='Test Points'),
    Line2D([0], [0], color = cmap(norm(14)), linestyle=':', label='Estimated Curve'),
...
#make legend compact
plt.title(f"Accuracy of the estimated Scaling Laws")
#tight layout
plt.tight_layout()
#save the plot
plt.savefig(f"plots_test/test_scaling_plot_text{TEXT_TOKENS}.png")
plt.savefig(f"plots_test/test_scaling_plot_text{TEXT_TOKENS}.pdf")

# estimate = func_error_fit(best_params, [14, 144])
# print(f"Estimated Accuracy for 14B params and 144 tokens: {1-estimate}")

# estimate = func_error_fit(best_params, [14, 36])
# print(f"Estimated Accuracy for 14B params and 36 tokens: {1-estimate}")

# estimate = func_error_fit(best_params, [14, 1])
# print(f"Estimated Accuracy for 14B params and 1 tokens: {1-estimate}")